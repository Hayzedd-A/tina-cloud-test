...
const AvailableBreads = () => {
  ...
  const searchBread = async () => {
    ...
    if (!searchQ) return setFilteredData(dataTemp);
    dataTemp = normalizeData(
      allRawData.filter(
        (x) =>
  ...
  const selectSize = async (e, size) => {
    for (
      let i = 0;
      i < document.getElementsByClassName("sizesLi").length;
      i++
    ) {
  ...
  useEffect(() => {
    let dataTemp = normalizeData(allRawData);
    if (selectedSize === "All") return setFilteredData(dataTemp);
  ...
  const checkSize = (size, e) => {
    ...
    filteredDataTmp.find((x) => x.size === size.size).checked =
      e.target.checked;

    let cartTmp = JSON.parse(JSON.stringify(cart));
    if (e.target.checked)
      filteredDataTmp
        .find((x) => x.size === size.size)
        .items.map((x) => {
  ...
  const checkItem = (size, item, e) => {
    ...
    else cartTmp = cartTmp.filter((x) => x.id !== item.id);
  ...
  const placeOrder = (source) => {
    ...
    if (source === "whatsapp") {
      ...
      const cartItems = localStorage.getItem("gourmettwistcart")
        ? JSON.parse(localStorage.getItem("gourmettwistcart"))
        : [];
      filteredData
        .filter(
          (x) => x.items.filter((i) => cart.find((c) => c.id === i.id)).length
        )
        .forEach((x) => {
          x.items
            ...
            .forEach((item) => {
              if (cartItems.find((c) => c.id === item.id)) {
                const newQty =
                  ...
                  parseInt(
                    cart.find((c) => c.id === item.id).quantityToPurchase
                  );
                cartItems[cartItems.findIndex((c) => c.id === item.id)] = {
                ...
                cartItems.push({
                  ...
                  name: item.name,
                  quantity: cart.find((c) => c.id === item.id)
                    .quantityToPurchase,
                  toppings: [],
                  totalCost:
                    cart.find((c) => c.id === item.id).quantityToPurchase *
      ...
      setShowCheckoutModal(
        `Added x${cartItems.length} item(s) successfully to the cart`
      );
    }
  };
  ...
  const changePurchaseQty = (size, item, e) => {
    if (
      filteredData
        .find((x) => x.size === size.size)
        .items.find((x) => x.id === item.id).stockQty < parseInt(e.target.value)
    )
      return message.warning(
        "Please do not provide quantity more than available stock"
      );
  ...
  return (
    <Main>
      ...
      <div style={{ display: "flex", marginTop: "3%" }}>
        ...
        <div style={{ flex: 1, fontSize: "14pt" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            />
            <input
              placeholder="Search with item name and size"
              style={{
                width: "100%",
                padding: 10,
                fontSize: "12pt",
                      ...
                      return (
                        <ul
                          ...
                          style={{
                            ...
                            paddingTop: "20px",
                            marginBottom:
                              d.items.length - 1 === idx ? "30px" : null,
                            borderBottom:
                              d.items.length - 1 === idx
                                ? "3px solid rgb(228 212 212)"
                                : null,
                            paddingBottom:
                              d.items.length - 1 === idx ? "30px" : null,
                          }}
                        >
                          ...
                          >
                            <Checkbox
                              checked={
                                cart.find((c) => c.id === i.id) ? true : false
                              }
      ...
      >
        ...
        <div
          className="floatButtonsContainer"
          onClick={() => {
            placeOrder("whatsapp");
        ...
        >
          <FloatButton
            style={{ display: "inline-block" }}
            tooltip={"Order via website"}
            icon={<img src="/static/images/splash-logo.png" />}
          />
      ...
      >
        <p>
          Order details are copied to clipboard. You will need to paste in the
          chatbox
        </p>
        <br />
        <p style={{ fontWeight: 600 }}>
          Opening instagram chat box in {timerRedirect}...
        </p>
      </Modal>
