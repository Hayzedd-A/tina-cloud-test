...
class FAQ extends Component {
    ...
    this.state = {
      ...
      faq: [
        { 
          ...
          A: "Pick-ups are available for pre-paid orders. Our pick-up location is RT Lawal Street, behind Meadow Hall School, Lekki."
        },
        { 
  ...
  render() {
    ...
    const { faq, selectedFAQ } = this.state;
          ...
          <div className="checkout-form login-form">
            <div className="container">
              <div className="faq-content">
                ...
                <p className="about-faq">Gourmet Twist is an online bakery/food store that is redefining your favorite foods with an epicurean twist. Our breads are baked fresh and meals served hot every day.</p>
                {
                  faq.map(({ Q, A }, index) => {
                    return (
                      <>
                        <div className={`question ${selectedFAQ === index ? 'active' : '' }`} onClick={() => this.setState({ selectedFAQ: index })}>
                          <h3>{index+1}. {Q}</h3>
                        </div>
                        <div className={`answer ${selectedFAQ === index ? 'active' : '' }`}>
                          <p>{A}</p>
                        </div>
                      </>
                    )
